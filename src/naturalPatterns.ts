...
export function createPatternRenderer(svg:SVGSVGElement){
 ...
 return {
  ...
  export(colors={fillColor:'#6CF6AEFF',outlineColor:'#A7FFD5FF',lineWidth:15,eyeFill:'#44CDDFFF',eyeOutline:'#D8FFF0FF',pupilColor:'#071014FF',eyeLineWidth:5.5,pupilSize:17,eyes:[{x:.42,y:.43},{x:.58,y:.43}]} as {fillColor:string;outlineColor:string;lineWidth:number;eyeFill:string;eyeOutline:string;pupilColor:string;eyeLineWidth:number;pupilSize:number;eyes:Point[]}){const copy=svg.cloneNode(true) as SVGSVGElement;copy.setAttribute('xmlns',NS);copy.removeAttribute('class');copy.removeAttribute('style');const sourceBody=svg.querySelector('path[fill="white"]') as SVGPathElement|null;if(sourceBody){const body=sourceBody.cloneNode(true) as SVGPathElement;body.setAttribute('fill',colors.fillColor);body.setAttribute('stroke',colors.outlineColor);body.setAttribute('stroke-width',String(colors.lineWidth));body.removeAttribute('mask');copy.appendChild(body)}copy.querySelector('mask')?.remove();const circles=Array.from(svg.querySelectorAll('circle'));circles.forEach((eye,i)=>{const c=eye.cloneNode(true) as SVGCircleElement;c.setAttribute('fill',colors.eyeFill);c.setAttribute('stroke',colors.eyeOutline);c.setAttribute('stroke-width',String(colors.eyeLineWidth));if(i<colors.eyes.length){c.setAttribute('cx',String(colors.eyes[i].x*Number(svg.getAttribute('width')||1100)));c.setAttribute('cy',String(colors.eyes[i].y*Number(svg.getAttribute('height')||900)));}copy.appendChild(c);const pupil=c.cloneNode(true) as SVGCircleElement;pupil.setAttribute('r',String(colors.pupilSize));pupil.setAttribute('fill',colors.pupilColor);pupil.setAttribute('stroke','none');copy.appendChild(pupil)});const blob=new Blob([new XMLSerializer().serializeToString(copy)],{type:'image/svg+xml'}),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download='soulspawn-character.svg';link.click();setTimeout(()=>URL.revokeObjectURL(url),1000)},
