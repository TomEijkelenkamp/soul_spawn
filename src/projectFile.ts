...
export type VisualStyle={lineWidth:number;outlineColor:string;fillColor:string;eyeOutline:string;eyeFill:string;pupilColor:string;eyeLineWidth:number;pupilSize:number}
export type SavedView={zoom:number;centerX:number;centerY:number}
export type Design={points:Point[];eyes:Point[];regions:PatternRegion[];selectedRegion:number;patternEdgeMargin:number;patternEyeMargin:number;pressure:number;stiffness:number;smoothing:number;contourLength:number;visual:VisualStyle;paused:boolean;structure:number;body:SavedParticle[];view:SavedView}
export function serializeDesign(design:Design){return JSON.stringify({app:'Soulspawn',version:1,design},null,2)}
...
export function parseDesign(text:string):Design{
 ...
 const file=object(parsed);if(file.app!=='Soulspawn')throw invalid();if(file.version!==1)throw new Error('Deze bestandsversie wordt nog niet ondersteund.')
 ...
 if(typeof d.paused!=='boolean'||!Array.isArray(d.body)||d.body.length!==64)throw invalid()
